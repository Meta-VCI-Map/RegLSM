...
%% Initialization 
    ...
    reg_FLAIR_T1=['-p ',blanks(1),'"',Dir_code,'\Parameter\affine.txt','"',' -p ',blanks(1),'"',Dir_code,'\Parameter\bspline.txt','"'];
...
para_name={'to_MNI.txt';'to_MNI_lesion.txt';'SC_to_MNI.txt';'SC_to_MNI.1.txt';...
           'TransformParameters.1.txt';'TransformParameters.0.txt';'FLAIR_to_T1.txt';'FLAIR_to_T1.1.txt';'T1_to_SC.txt';'T1_to_SC.1.txt';};
...
clc;
    ...
    FLAIR_to_T1=strcat(Dir,'\FLAIR_to_T1\TransformParameters.1.txt');
    copyfile(FLAIR_to_T1,dir_tar);
    movefile(strcat(dir_tar,'TransformParameters.1.txt'),strcat(dir_tar,'FLAIR_to_T1.txt'));
    str_t1=strcat(dir_tar,'FLAIR_to_T1.txt');
    
    FLAIR_to_T1_1=strcat(Dir,'\FLAIR_to_T1\TransformParameters.0.txt');
    copyfile(FLAIR_to_T1_1,dir_tar);
    movefile(strcat(dir_tar,'TransformParameters.0.txt'),strcat(dir_tar,'FLAIR_to_T1.1.txt'));
    str_t1_1=strcat(dir_tar,'FLAIR_to_T1.1.txt');
    ...
    % Rewrite FLAIR_to_T1_affine transformation file with initial registration of T1 to Rorden
    fid_m=fopen(str_t1_1);
    count=1;
    str=[];
    while ~feof(fid_m)
          tline = fgetl(fid_m);
    ...
    fid_m = fopen(str_t1_1,'wt');
    for k=1:(count-1)
        fprintf(fid_m, '%s\n', str{k});
    end
    fclose(fid_m);
    
    % Rewrite FLAIR_to_T1_bspline for the current directory
    fid_m=fopen(str_t1);
    count=1;
    str=[];
    while ~feof(fid_m)
    ...
    for k=1:(count-1)        
        str{k}=strrep(str{k},strcat(Dir,'\FLAIR_to_T1\TransformParameters.0.txt'),str_t1_1);
    end
